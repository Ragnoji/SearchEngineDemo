...
class AND(object):
    ...
    def matches(self, index):
        s = self.parts[0].matches(index)
        for i in range(1, len(self.parts)):
            part = self.parts[i]
            s = s & part.matches(index)
            if not s:
                self.last = False
                return set()
        self.last = True
...
class NOT(object):
    ...
    def matches(self, index):
        others = reduce(lambda x, y: x | y, index.values())
        return others - self.tree.matches(index)
...
class Token(object):
    ...
    def __init__(self, string):
        ...
        self.string = morph.parse(string)[0].normal_form
        print(self.string)
